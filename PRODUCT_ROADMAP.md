...
- [x] **Гейтинг за статтю.** При `Gender == Male` фіча недоступна цілком: блок налаштувань зникає, секція
  дня не показується. Прапорець і записані дні при цьому **не стираються**, тож помилковий вибір оборотний.
