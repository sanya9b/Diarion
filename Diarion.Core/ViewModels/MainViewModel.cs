...
public partial class MainViewModel : BaseViewModel
{
    ...
    public async Task FlushAutoSaveAsync()
    {
        if (_autoSaveCts != null && !_autoSaveCts.IsCancellationRequested)
        {
            ...
            {
                await _diaryService.SaveEntryAsync(CurrentEntry);
                System.Diagnostics.Debug.WriteLine($"Flush Auto-saved entry for {CurrentEntry.Date:dd.MM.yyyy}");
            }
        }
    }
    ...
    public MainViewModel(IDiaryService diaryService, IProfileService profileService, ITodoService todoService, IHabitService habitService, IMenstrualCycleService menstrualCycleService, ICalendarService calendarService, IMenuConfigurationService menuConfigurationService)
    {
