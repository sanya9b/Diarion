...
2. **Themes (Dark/Light):** Обов'язкова підтримка світлого та темного режимів. Використовуй `ResourceDictionaries` (наприклад, `Colors.xaml`, `Styles.xaml`) з використанням `AppThemeBinding`.
3. **Color Palette:** Використовуй заспокійливі природні відтінки (кремові, бежеві, оливкові, глибокий синьо-сірий, темно-сірий для контрасту) та акцентний теракотовий колір (Coral, `#C26D53`) для інтерактивних штук, наприклад, зірочок рейтингу.
4. **Monochromatic Style:** Усі UI елементи (кнопки, бейджі, іконки, статуси) мають бути суворо монохромними або використовувати палітру. Уникай використання яскравих різнокольорових маркерів (червоний для помилок/високого пріоритету, зелений для успіху). Використовуй виключно базову палітру.
